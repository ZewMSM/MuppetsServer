...
async def buy_structure(client: SFSServerClient, params: SFSObject):
    ...
    if active_island is None:
        return SFSObject().putBool('success', False).putUtfString('message', 'No active island')

    structure = await Structure.load_by_id(structure_id)
    if structure is None:
        return SFSObject().putBool('success', False).putUtfString('message', 'Invalid structure')

    if structure.structure_type not in ('decoration', 'bakery'):
        if active_island.has_structure_type_id(structure_id):
            return SFSObject().putBool('success', False).putUtfString('message', 'Error buying stricture')
    ...
    user_structure = await PlayerStructure.create_new_structure(
        active_island.id, structure_id, pos_x, pos_y,
        completed=True, flip=int(flip) if flip else 0, scale=float(scale) if scale else 1.0,
    )
...
async def move_structure(client: SFSServerClient, params: 'SFSObject'):
    ...
    user_structure = client.player.get_active_island.get_structure(user_structure_id) if client.player.get_active_island else None
    if user_structure is None:
        return SFSObject().putBool('success', False).putUtfString('message', 'Error move stricture')
    ...
    props.addSFSObject(SFSObject().putFloat('scale', scale))
...
async def mute_structure(client: SFSServerClient, params: 'SFSObject'):
    ...
    user_structure = client.player.get_active_island.get_structure(user_structure_id) if client.player.get_active_island else None
    if user_structure is None:
        return SFSObject().putBool('success', False).putUtfString('message', 'Error mute stricture')
    ...
    await client.send_extension("gs_mute_structure", SFSObject().putBool('success', True))
...
async def flip_structure(client: 'SFSServerClient', params: 'SFSObject'):
    ...
    user_structure = client.player.get_active_island.get_structure(user_structure_id) if client.player.get_active_island else None
    if user_structure is None:
        return SFSObject().putBool('success', False).putUtfString('message', 'Error flip stricture')
    ...
    await client.send_extension("gs_flip_structure", SFSObject().putBool('success', True))
    await client.send_extension("gs_update_structure", update_resp)
...
async def sell_structure(client: SFSServerClient, params: 'SFSObject'):
    ...
    user_structure = client.player.get_active_island.get_structure(user_structure_id) if client.player.get_active_island else None
    if user_structure is None:
        return SFSObject().putBool('success', False).putUtfString('message', 'Error sell stricture')
...
async def start_upgrade_structure(client: SFSServerClient, params: 'SFSObject'):
    ...
    user_structure = client.player.get_active_island.get_structure(user_structure_id) if client.player.get_active_island else None
    if user_structure is None:
        return SFSObject().putBool('success', False).putUtfString('message', 'Error upgrading stricture')
    ...
    user_structure.building_completed = int(time.time() * 1000) + new_structure.build_time * 1000
    await user_structure.save()
...
async def clear_obstacle(client: SFSServerClient, params: 'SFSObject'):
    ...
    user_structure = client.player.get_active_island.get_structure(user_structure_id) if client.player.get_active_island else None
    if user_structure is None:
        return SFSObject().putBool('success', False).putUtfString('message', 'Error clear obstacle')
...
async def start_obstacle(client: SFSServerClient, params: SFSObject):
    ...
    user_structure = client.player.get_active_island.get_structure(user_structure_id) if client.player.get_active_island else None
    if user_structure is None:
        return SFSObject().putBool('success', False).putUtfString('message', 'Error start obstacle')
    ...
    user_structure.building_completed = now_ms + user_structure.structure.build_time * 1000
