...
async def buy_island(client: SFSServerClient, params: SFSObject):
    ...
    if island_id == 0:
        return SFSObject().putBool('success', False).putUtfString('message', 'Error buying island')
    ...
    if client.player.get_island_by_id(island_id) is not None:
        return SFSObject().putBool('success', False).putUtfString('message', 'User already have this island')
...
async def change_island(client: SFSServerClient, params: SFSObject):
    ...
    if client.player.get_island(user_island_id) is None:
        return SFSObject().putBool('success', False).putUtfString('message', 'User don\'t have this island')
