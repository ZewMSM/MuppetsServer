...
@register_adapter(BackdropDB)
class Backdrop(BaseAdapter):
    _db_model = BackdropDB
    _game_id_key = "backdrop_id"
    ...
    async def _ensure_loaded(cls):
        async with _session_ctx() as session:
            ...
            if result.scalar_one_or_none() is not None:
                return
        await cls.import_from_json(_BACKDROP_JSON_PATH)
    ...
    async def import_from_json(cls, path: Path):
        if not path.exists():
            logger.warning("Backdrop JSON not found: %s", path)
            return
        raw = path.read_text(encoding="utf-8")
        data = json.loads(raw)
        items = data.get("backdrop_data", [])
        db_rows = []
        for record in items:
            backdrop_id = record.get("backdrop_id", 0)
            inst = cls()
            inst.id = backdrop_id
            inst.backdrop_id = backdrop_id
        ...
        logger.info("Loaded %d backdrops from %s", len(items), path)

    async def update_sfs(self, params: SFSObject):
        params.remove_item("date_created")
        params.putInt("backdrop_id", self.id)
